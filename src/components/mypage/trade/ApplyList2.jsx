...
import classes from "./ApplyList.module.css";
...
export default function ApplyList() {
  ...
  if (orderFilter === "grade") {
    data.forEach((tempList) => {
      tempList.grade === "BRONZE"
        ? (tempList.grade = 1)
        : tempList.grade === "SILVER"
        ? (tempList.grade = 2)
        : tempList.grade === "GOLD"
        ? (tempList.grade = 3)
        : tempList.grade === "PLATINUM"
        ? (tempList.grade = 4)
        : tempList.grade === "DIAMOND"
        ? (tempList.grade = 5)
        : console.log("미등록");
    });
    data.sort((a, b) => (a[orderFilter] > b[orderFilter] ? direction : direction * -1));
  } else {
    data.sort((a, b) => (a[orderFilter] > b[orderFilter] ? direction : direction * -1));
  }
  ...
  const onReCheck = (targetId) => {
    // 신청상태(ing: 진행, 검토: exa, 승인:app, 거절: ref ) 바꿔주는 코드 작성 "해" "줘"!
  };
  ...
  const onloadApplyList = async () => {
    try {
      const getData = await loadApplyWatches(memberId);
                  ...
                  <div className={classes.product}>
                    ...
                    <div>
                      <ul>
                        ...
                        <li className={classes.other}>상품 금액 : {watch.price}원</li>
