...
import classes from "./QuestionListAdmin.module.css";
...
const QuestionListAdmin = () => {
  ...
  // 질문 조회
  const loadQuestionList = async (param) => {
    // console.log("param : ", param);
    if (param === 0) {
      try {
        const getData = await loadAllQuestion();
    ...
    if (param === 1) {
      try {
        const getData = await loadNoneList();
        setData(getData.data);
        setIsLoading(true);
      } catch (e) {}
    }
    if (param === 2) {
      try {
        const getData = await loadAnswerList();
        setData(getData.data);
        setIsLoading(true);
      } catch (e) {}
    }
  ...
  const loadQuestionCount = async (param) => {
    ...
    if (param === 1) {
      try {
        const getData = await loadToggleCount(0);
    ...
    if (param === 2) {
      try {
        const getData = await loadToggleCount(1);
  ...
  const initCount = () => {
    loadQuestionCount(1);
    loadQuestionCount(2);
    setAllCount(noAnswerCount + answerCount);
  };
  useEffect(() => {
      initCount();
      ...
      <div className={classes.root}>
        ...
        <div className={classes.middle}>
          ...
          <div className={classes.lefttop}>
            <h3 style={{ fontWeight: "bold" }}>제목</h3>
          </div>
          {/* 오른쪽 상단 버튼 */}
            ...
            {data
              .filter((question) =>
                new Date(question.question_date) > new Date(startDate)
                  ? true
                  : new Date(question.question_date).getDate() >=
                    startDate.getDate()
                  ? true
                  : false &&
                    new Date(question.question_date) <= new Date(endDate)
              )
                  ...
                  <div htmlFor={`select-${question.question_id}`}>
                    ...
                    <div className={classes.product}>
                      ...
                      <input
                        ...
                        checked={
                          checkItems.includes(question.question_id)
                            ? true
                            : false
                        }
                      />
